...
function showResult() {
  ...
  if (notaIngresada > 0 && notaIngresada <= 100) {
    ...
    document.querySelector(
      "#pResult"
    ).innerHTML = `Alumnos que perdieron:${perdieron}<br> Alumnos que salvaron: ${salvaron}<br> Muy buena nota: ${muyBuenaNota}<br> Promedio de notas: ${
      acumulaNotas / contAlumnosCargados
    }<br> Nota maxima: ${maxNota}<br> Nota minima: ${minNota}`;
  } else {
    document.querySelector("#pResult").innerHTML =
      "Nota invalida, debe ser entre 1 a 100";
  }
}
